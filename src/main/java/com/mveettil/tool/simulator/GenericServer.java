...
public class GenericServer implements Runnable {
	...
	try {
	    ...
	    InputStream props = Thread.currentThread().getContextClassLoader()
    ...
    private static String readXmlFile(String fname) throws IOException {

	InputStream fxml = Thread.currentThread().getContextClassLoader()
	...
	Runtime.getRuntime().addShutdownHook(new Thread() {
	    public void run() {
		logger.warn("Caught <ctrl-c>, shutting DayTime Service down!");
		service.interrupt();
	    }
	});
    }
	...
	try {
	    ServerSocket serverSocket = new ServerSocket(
		    Integer.parseInt(GenericServer.port));
	    try {
...
class Worker implements Runnable {
	...
	try {
		...
		out.println(GenericServer.respXml);
